...
double * read_data(char * filename, int nTime, int nLat, int nLon) {
  FILE * fp;
...
int main(void) {
  printf("Testing eofunc_ts_n (01): ... \n");
  int nTime = 127;
  int nLat  =  91;
  int nLon  = 180;
  char * data_filename = "resources/SampleSST.csv";
  char * expected_ev_filename = "resources/SampleSST_ev.txt";
  char * expected_ev_ts_filename = "resources/SampleSST_ev_ts.txt";

  double * data = read_data(data_filename, nTime, nLat, nLon);
  ...
  printf("Testing eofunc_n output ...\n");
  if (ev->ndim != 3) {
  ...
  printf("Testing eofunc_ts_n output: ...\n");
  if (ev_ts->ndim != 2) {
