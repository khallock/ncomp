...
extern void drcm2rgrid_(int *ingrid, int *inlat2d, int *inlon2d, double *tmp_lat2d, double *tmp_lon2d,
						...
						double *tmp_lon1d, double *tmp_fo, double *missing_dfi,
						int *tmp_ncrit, int *tmp_opt, int *ier);

extern void	drgrid2rcm_(int *ingrid, int *inlat1d, int *inlon1d, double *tmp_lat1d, double *tmp_lon1d,
						double *tmp_fi, int *inlat2d, int *inlon2d, double *tmp_lat2d,
						double *tmp_lon2d, double *tmp_fo, double *missing_dfi,
						int *tmp_ncrit, int *tmp_opt, int *ier);
			  ...
			  const ncomp_array* lat1d, const ncomp_array* lon1d, ncomp_array* fo)
{
	...
	if(lat2d->shape[0] != lon2d->shape[0] ||
	   lat2d->shape[1] != lon2d->shape[1]) {
		std::cerr << "ERROR rcm2rgrid: The input lat/lon grids must be the same size ! \n";
	}
	...
	if(nlon2d <= 1 || nlat2d <= 1 || nlat1d <= 1 || nlon1d <= 1) {
		std::cerr << "ERROR rcm2rgrid: The input/output lat/lon grids must have at least 2 elements ! \n";
	}
	...
	if(fo->type == NCOMP_DOUBLE) {
		...
		std::vector<double> tmp_fo_vec(size_fo);
	...
	/**************** START: FORTRAN CALL ****************/
	drcm2rgrid_(&ingrid, &inlat2d, &inlon2d, tmp_lat2d, tmp_lon2d,
				tmp_fi, &inlat1d, tmp_lat1d, &inlon1d,
				tmp_lon1d, tmp_fo, &missing_dfi,
				&tmp_ncrit, &tmp_opt, &ier);
	/**************** END: FORTRAN CALL ****************/

	if(ier) {
		if(ier == 1) {
			std::cerr << "ERROR rcm2rgrid: not enough points in input/output array ! \n";
		}
		if(2 <= ier && ier <= 5) {
