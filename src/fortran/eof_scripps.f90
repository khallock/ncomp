...
subroutine DEOF(data, nx, nt, nmodes, icovcor, dmsg, &
  ...
  ! c c       print *, 'sqrootweights:',sqrootweights(1:10)
  END IF
  ...
  if( nmodes .gt. orderofs ) then
    ...
    print * , 'modes than exist!  Request=',nmodes,' exist=', orderofs
    stop 'eof '
  endif
  ...
  if( doswitched ) then
    ...
    do jascending=1, nmodes
      ...
      do i=1, nx
        eigenvectors(i,jdescending) = tentpcs(i,jascending)*fact
      end do
    end do
    ...
    do jascending=1, nmodes
      ...
      do i=1, nt
        princomp(i,jdescending) = evecs(i,jascending)*fact
      end do
    end do
  else
    ! 	-------------------------------------------------
    ...
    do jascending=1, nmodes
      jdescending = nmodes - jascending + 1
      do i=1, nx
        eigenvectors(i,jdescending) = evecs(i,jascending)
      end do
    end do
    ...
    do jascending=1, nmodes
      jdescending = nmodes - jascending + 1
      do i=1, nt
        princomp(i,jdescending) = tentpcs(i,jascending)
      end do
    end do
  endif
  ...
  do mode=1, nmodes
    do i=1, nx
      if( sqrootweights(i) .eq. 0.0d0 ) then
        ...
        eigenvectors(i,mode) = eigenvectors(i,mode) / sqrootweights(i)
      endif
    end do
  end do
  ...
  do jascending=nmodes, 1, -1
    ...
    variance(jdescending)    = eigenvalues(jdescending)/totvar*100.0d0
