...
ncomp_array* ncomp_array_alloc_missing(void* array_ptr, int array_type, int array_ndim, size_t* array_shape, int has_missing, ncomp_missing msg) {
    ncomp_array* new_array = ncomp_array_alloc(array_ptr, array_type, array_ndim, array_shape);
    new_array->has_missing = 1;
    switch(array_type) {
        case NCOMP_FLOAT:
            new_array->msg.msg_float = msg.msg_float;
            break;
        case NCOMP_DOUBLE:
            new_array->msg.msg_double = msg.msg_double;
            break;
        case NCOMP_BOOL:
            new_array->msg.msg_bool = msg.msg_bool;
            break;
        case NCOMP_BYTE:
            new_array->msg.msg_byte = msg.msg_byte;
            break;
        case NCOMP_UBYTE:
            new_array->msg.msg_ubyte = msg.msg_ubyte;
            break;
        case NCOMP_SHORT:
            new_array->msg.msg_short = msg.msg_short;
            break;
        case NCOMP_USHORT:
            new_array->msg.msg_ushort = msg.msg_ushort;
            break;
        case NCOMP_INT:
            new_array->msg.msg_int = msg.msg_int;
            break;
        case NCOMP_UINT:
            new_array->msg.msg_uint = msg.msg_uint;
            break;
        case NCOMP_LONG:
            new_array->msg.msg_long = msg.msg_long;
            break;
        case NCOMP_ULONG:
            new_array->msg.msg_ulong = msg.msg_ulong;
            break;
        case NCOMP_LONGLONG:
            new_array->msg.msg_longlong = msg.msg_longlong;
            break;
        case NCOMP_ULONGLONG:
            new_array->msg.msg_ulonglong = msg.msg_ulonglong;
            break;
        case NCOMP_LONGDOUBLE:
            new_array->msg.msg_longdouble = msg.msg_longdouble;
            break;
    }
 ...
 */
    switch(type_x) {
        case NCOMP_DOUBLE:
            *missing_dx = missing_x->msg_double;
            break;
        case NCOMP_FLOAT:
            *missing_dx = (double) missing_x->msg_float;
            break;
        case NCOMP_BOOL:
            *missing_dx = (double) missing_x->msg_bool;
            break;
        case NCOMP_BYTE:
            *missing_dx = (double) missing_x->msg_byte;
            break;
        case NCOMP_UBYTE:
            *missing_dx = (double) missing_x->msg_ubyte;
            break;
        case NCOMP_SHORT:
            *missing_dx = (double) missing_x->msg_short;
            break;
        case NCOMP_USHORT:
            *missing_dx = (double) missing_x->msg_ushort;
            break;
        case NCOMP_INT:
            *missing_dx = (double) missing_x->msg_int;
            break;
        case NCOMP_UINT:
            *missing_dx = (double) missing_x->msg_uint;
            break;
        case NCOMP_LONG:
            *missing_dx = (double) missing_x->msg_long;
            break;
        case NCOMP_ULONG:
            *missing_dx = (double) missing_x->msg_ulong;
            break;
        case NCOMP_LONGLONG:
            *missing_dx = (double) missing_x->msg_longlong;
            break;
        case NCOMP_ULONGLONG:
            *missing_dx = (double) missing_x->msg_ulonglong;
            break;
        case NCOMP_LONGDOUBLE:
            *missing_dx = (double) missing_x->msg_longdouble;
            break;
    }
    ...
    if(type_x != NCOMP_DOUBLE && missing_rx != NULL) {
        switch(type_x) {
            case NCOMP_FLOAT:
                *missing_rx = missing_x->msg_float;
                break;
            case NCOMP_BOOL:
                *missing_rx = (float) missing_x->msg_bool;
                break;
            case NCOMP_BYTE:
                *missing_rx = (float) missing_x->msg_byte;
                break;
            case NCOMP_UBYTE:
                *missing_rx = (float) missing_x->msg_ubyte;
                break;
            case NCOMP_SHORT:
                *missing_rx = (float) missing_x->msg_short;
                break;
            case NCOMP_USHORT:
                *missing_rx = (float) missing_x->msg_ushort;
                break;
            case NCOMP_INT:
                *missing_rx = (float) missing_x->msg_int;
                break;
            case NCOMP_UINT:
                *missing_rx = (float) missing_x->msg_uint;
                break;
            case NCOMP_LONG:
                *missing_rx = (float) missing_x->msg_long;
                break;
            case NCOMP_ULONG:
                *missing_rx = (float) missing_x->msg_ulong;
                break;
            case NCOMP_LONGLONG:
                *missing_rx = (float) missing_x->msg_longlong;
                break;
            case NCOMP_ULONGLONG:
                *missing_rx = (float) missing_x->msg_ulonglong;
                break;
            case NCOMP_LONGDOUBLE:
                *missing_rx = (float) missing_x->msg_longdouble;
                break;
        }
    }
